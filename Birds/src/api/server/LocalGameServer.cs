...
public class LocalGameServer(GameController gameController, Dictionary<string, Player> players)
{
  ...
  private Dictionary<string, Player> _players = players;
  private const float CULLING_MARGIN = 500f;
  ...
  private GameStateMessage CollectDirtyStateForPlayer(string playerId, Camera camera, float cameraZoom)
  {
    ...
    {
      if (!camera.IsEntityWithinFrame(entity, cameraZoom, buffer: CULLING_MARGIN))
        continue;
