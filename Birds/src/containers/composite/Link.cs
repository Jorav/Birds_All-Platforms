...
public class Link
{
  ...
  public Vector2 RelativePositionRotated
  {
    get
    {
      return RelativePosition.Length() * Scale * new Vector2(
        (float)Math.Cos(MathHelper.WrapAngle(LinkRotation + Entity.Rotation.Value)),
        (float)Math.Sin(MathHelper.WrapAngle(LinkRotation + Entity.Rotation.Value))
      );
    }
  }

  public Vector2 AbsolutePositionOnEntity
  {
    get { return Entity.Position.Value + RelativePositionRotated; }
  }

  public Vector2 GetConnectionPosition(Link otherLink)
  {
    ...
    float otherLinkLength = otherLink.RelativePosition.Length() * otherLink.Scale;
    return AbsolutePositionOnEntity + dir * otherLinkLength;
  }
