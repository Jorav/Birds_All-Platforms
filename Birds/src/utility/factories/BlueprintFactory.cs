...
public static class BlueprintFactory
{
  ...
  private static void ConnectEntities(List<Connection> connections, Dictionary<int, WorldEntity> entityLookup)
  {
    foreach (var connection in connections)
    {
      ...
      {
        entity2LinkModule.ConnectAgainst(entity1, link2, link1);
      }
    }
  }
  ...
  public static void RestoreConnections(List<IEntity> originalEntities, List<IEntity> clonedEntities)
  {
    ...
    {
      var originalEntity = originalEntities[entityIndex];
      var originalLinkModule = originalEntity.GetModule<LinkModule>();

      var clonedLinkModule = clonedEntities[entityIndex].GetModule<LinkModule>();

      for (int linkIndex = 0; linkIndex < originalLinkModule.Links.Count; linkIndex++)
      {
        var originalLink = originalLinkModule.Links[linkIndex];
        if (originalLink.ConnectionAvailable)
          continue;
        ...
        var connectedOriginalLinkModule = connectedEntity.GetModule<LinkModule>();
        var connectedLinkIndex = connectedOriginalLinkModule.Links.IndexOf(originalLink.connection);
        var connectedClonedLinkModule = clonedEntities[connectedEntityIndex].GetModule<LinkModule>();

        var clonedLink1 = clonedLinkModule.Links[linkIndex];
        var clonedLink2 = connectedClonedLinkModule.Links[connectedLinkIndex];
